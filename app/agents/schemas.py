...
class RequestImageUploadInput(BaseModel):
    customer_email: EmailStr = Field(
        ...
        description=(
            ...
            "MUST NOT be invented, defaulted, or constructed from the "
            "customer's name (e.g. firstname.lastname@gmail.com). "
            "Placeholder domains (example.com, test.com) will be rejected. "
            "Name-derived emails will also be rejected unless explicitly "
            "confirmed by the customer."
        ),
    )
    customer_confirmed_aloud: bool = Field(
        ...,
        description=(
            "REQUIRED. Set to True ONLY if you have: "
            "(1) asked the customer for their email aloud, "
            "(2) heard them speak it back, "
            "(3) read it back to them letter-by-letter for confirmation, "
            "(4) received an explicit 'yes' or equivalent. "
            "If False, the tool will refuse and ask you to follow the protocol. "
            "DO NOT set this to True if you guessed the email — the customer "
            "must have spoken it themselves."
        ),
    )
